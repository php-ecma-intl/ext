...
#define TEST_SUITE ecma402LocaleGetHourCycle
...
ParameterizedTestParameters(TEST_SUITE, getHourCycle)
{
	...
	STRING_TEST("de-latn-de-u-ca-gregory-co-phonebk-hc-h23-kf-true-kn-false-nu-latn", "h23")
	STRING_TEST("ja-jpan-jp-u-ca-japanese-co-search-hc-h24-kf-false-kn-nu-jpanfin", "h24")
	STRING_TEST("fr-latn-ca-u-ca-gregory-co-standard-hc-h11-kf-kn-false-nu-latn", "h11")
...
ParameterizedTest(stringTestParams *test, TEST_SUITE, getHourCycle)
{
	...
	const int resultLength = ecma402_getHourCycle(test->input, result, status, false);
	...
	if (strcmp(test->expected, "-1") == 0) {
		...
		cr_expect(eq(str, result, test->expected), "Expected locale \"%s\" hours of \"%s\"; got \"%s\" instead",
...
Test(TEST_SUITE, getHourCycleReturnsNegativeOneForNullPointer)
{
	...
	char *result = (char *)malloc(sizeof(char) * ULOC_KEYWORDS_CAPACITY);
	const int resultLength = ecma402_getHourCycle(NULL, result, status, false);
