...
ParameterizedTestParameters(TEST_SUITE, getCalendar)
{
	...
	STRING_TEST("de-latn-de-u-ca-gregory-co-phonebk-hc-h23-kf-true-kn-false-nu-latn", "gregory")
	STRING_TEST("ja-jpan-jp-u-ca-japanese-co-search-hc-h24-kf-false-kn-nu-jpanfin", "japanese")
	STRING_TEST("fr-latn-ca-u-ca-gregory-co-standard-hc-h11-kf-kn-false-nu-latn", "gregory")
...
ParameterizedTest(stringTestParams *test, TEST_SUITE, getCalendar)
{
	...
	const int resultLength = ecma402_getCalendar(test->input, result, status, false);
	...
	if (strcmp(test->expected, "-1") == 0) {
		...
		cr_expect(eq(str, result, test->expected), "Expected locale \"%s\" calendar of \"%s\"; got \"%s\" instead",
...
Test(TEST_SUITE, getCalendarReturnsNegativeOneForNullPointer)
{
	...
	char *result = (char *)malloc(sizeof(char) * ULOC_KEYWORDS_CAPACITY);
	const int resultLength = ecma402_getCalendar(NULL, result, status, false);
