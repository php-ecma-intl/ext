...
ParameterizedTestParameters(TEST_SUITE,
                            isStructurallyValidLanguageTagReturnsFalse) {
...
ParameterizedTest(string *test, TEST_SUITE,
  ...
  cr_expect(eq(i8, ecma402_isStructurallyValidLanguageTag((*test).c_str()), 0),
            "Expected \"%s\" to be invalid language tag, but it is valid",
            (*test).c_str());
}
...
ParameterizedTest(string *test, TEST_SUITE, isUnicodeVariantSubtagReturnsTrue) {
  cr_expect(eq(i8, ecma402::isUnicodeVariantSubtag(test->c_str()), true),
            "Expected true for \"%s\"; received false", test->c_str());
}
...
ParameterizedTest(string *test, TEST_SUITE,
                  isUnicodeVariantSubtagReturnsFalse) {
  cr_expect(eq(i8, ecma402::isUnicodeVariantSubtag(test->c_str()), false),
            "Expected false for \"%s\"; received true", test->c_str());
}
...
ParameterizedTest(string *test, TEST_SUITE, parseUnicodeLanguageIdReturnsTrue) {
  ...
  cr_expect(eq(i8, parser.parseUnicodeLanguageId(), true),
            "Expected true for \"%s\"; received false", test->c_str());
}
...
ParameterizedTest(string *test, TEST_SUITE,
                  parseUnicodeLanguageIdReturnsFalse) {
  ecma402::LanguageTagParser parser(test->c_str());
  cr_expect(eq(i8, parser.parseUnicodeLanguageId(), false),
            "Expected false for \"%s\"; received true", test->c_str());
}
...
ParameterizedTest(string *test, TEST_SUITE, parseUnicodeLocaleIdReturnsTrue) {
  ecma402::LanguageTagParser parser(test->c_str());
  cr_expect(eq(i8, parser.parseUnicodeLocaleId(), true),
            "Expected true for \"%s\"; received false", test->c_str());
}
...
ParameterizedTest(string *test, TEST_SUITE, parseUnicodeLocaleIdReturnsFalse) {
  ecma402::LanguageTagParser parser(test->c_str());
  cr_expect(eq(i8, parser.parseUnicodeLocaleId(), false),
            "Expected false for \"%s\"; received true", test->c_str());
}
