...
ParameterizedTestParameters(TEST_SUITE, isNumeric)
{
	...
	STRING_TEST("de-latn-de-u-ca-gregory-co-phonebk-hc-h23-kf-true-kn-false-nu-latn", "false")
	STRING_TEST("ja-jpan-jp-u-ca-japanese-co-search-hc-h24-kf-false-kn-nu-jpanfin", "true")
	STRING_TEST("fr-latn-ca-u-ca-gregory-co-standard-hc-h11-kf-kn-true-nu-latn", "true")
...
ParameterizedTest(stringTestParams *test, TEST_SUITE, isNumeric)
{
	...
	if (strcmp(test->expected, "false") == 0) {
		cr_expect(eq(i8, result, 0), "Expected false numeric for \"%s\"", test->input);
	} else {
