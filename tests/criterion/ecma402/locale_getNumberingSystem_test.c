...
#define TEST_SUITE ecma402LocaleGetNumberingSystem
...
ParameterizedTestParameters(TEST_SUITE, getNumberingSystem)
{
	...
	STRING_TEST("ja-jpan-jp-u-ca-japanese-co-search-hc-h24-kf-false-kn-nu-jpanfin", "jpanfin")
...
ParameterizedTest(stringTestParams *test, TEST_SUITE, getNumberingSystem)
{
	...
	const int resultLength = ecma402_getNumberingSystem(test->input, result, status, false);
	...
	if (strcmp(test->expected, "-1") == 0) {
		...
		cr_expect(eq(str, result, test->expected), "Expected locale \"%s\" numbers of \"%s\"; got \"%s\" instead",
...
Test(TEST_SUITE, getNumberingSystemReturnsNegativeOneForNullPointer)
{
	...
	char *result = (char *)malloc(sizeof(char) * ULOC_KEYWORDS_CAPACITY);
	const int resultLength = ecma402_getNumberingSystem(NULL, result, status, false);
