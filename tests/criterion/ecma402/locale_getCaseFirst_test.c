...
#define TEST_SUITE ecma402LocaleGetCaseFirst
...
ParameterizedTestParameters(TEST_SUITE, getCaseFirst)
{
	...
	STRING_TEST("de-latn-de-u-ca-gregory-co-phonebk-hc-h23-kf-true-kn-false-nu-latn", "yes")
	STRING_TEST("ja-jpan-jp-u-ca-japanese-co-search-hc-h24-kf-false-kn-nu-jpanfin", "false")
	STRING_TEST("fr-latn-ca-u-ca-gregory-co-standard-hc-h11-kf-kn-false-nu-latn", "yes")
...
ParameterizedTest(stringTestParams *test, TEST_SUITE, getCaseFirst)
{
	...
	const int resultLength = ecma402_getCaseFirst(test->input, result, status, false);
	...
	if (strcmp(test->expected, "-1") == 0) {
		...
		cr_expect(eq(str, result, test->expected), "Expected locale \"%s\" colcasefirst of \"%s\"; got \"%s\" instead",
...
Test(TEST_SUITE, getCaseFirstReturnsNegativeOneForNullPointer)
{
	...
	char *result = (char *)malloc(sizeof(char) * ULOC_KEYWORDS_CAPACITY);
	const int resultLength = ecma402_getCaseFirst(NULL, result, status, false);
