...
static int addTest(struct bestAvailableTest *tests, int index,
                   const char *localeId, bool isCanonicalized,
...
void freeTests(struct criterion_test_params *criterionParams) {
  struct bestAvailableTest *tests =
      (struct bestAvailableTest *)criterionParams->params;
...
ParameterizedTestParameters(TEST_SUITE, bestAvailableLocale) {
  ...
  tests = (struct bestAvailableTest *)cr_malloc(
      sizeof(struct bestAvailableTest) * 100);
  ...
  idx = addTest(tests, idx, "en_US@calendar=gregory;collation=emoji", true,
                NULL, -1);
  idx = addTest(tests, idx, "en_US@calendar=gregory;collation=emoji", false,
                "en-US", 5);
...
ParameterizedTest(struct bestAvailableTest *test, TEST_SUITE,
                  bestAvailableLocale) {
  ...
  int resultLength = ecma402_bestAvailableLocale(
      available, count, test->localeId, result, test->isCanonicalized);

  cr_expect(eq(i8, resultLength, test->expectedLength),
            "Expected length of %d for localeId \"%s\"; received %d",
            test->expectedLength, test->localeId, resultLength);

  if (test->expected != NULL) {
    cr_expect(eq(str, result, test->expected),
              "Expected value of \"%s\" for localeId \"%s\"; received \"%s\"",
