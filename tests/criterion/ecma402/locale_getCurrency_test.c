...
ParameterizedTest(stringTestParams *test, TEST_SUITE, getCurrency)
{
	...
	if (strcmp(test->expected, "-1") == 0) {
		cr_assert(eq(i8, resultLength, -1), "Unexpected length returned for locale %s", test->input);
	} else {
		cr_assert(eq(str, result, test->expected), "Expected locale \"%s\" currency of \"%s\"; got \"%s\" instead",
		          test->input, test->expected, result);
		cr_assert(eq(i8, resultLength, strlen(test->expected)),
		          "Expected length %d for locale currency \"%s\"; got %d instead "
		          "for locale \"%s\"",
		          strlen(test->expected), test->expected, resultLength, test->input);
	}
