...
zend_object *ecma_createIntlLocaleWeekInfo(zend_class_entry *classEntry) {
  ...
  intlLocaleWeekInfo =
      zend_object_alloc(sizeof(struct ecma_IntlLocaleWeekInfo), classEntry);
...
PHP_METHOD(Ecma_Intl_Locale_WeekInfo, __construct) {
  ...
  if (!EXPECTED(Z_TYPE_P(weekendItem) == IS_OBJECT) ||
      !EXPECTED(instanceof_function(Z_OBJCE_P(weekendItem),
                                    ecma_ce_IntlLocaleWeekDay) != 0)) {
    zend_argument_type_error(
        2,
        "must be of type array<Ecma\\Intl\\Locale\\WeekDay>, %s found in array",
  ...
  zend_update_property(ecma_ce_IntlLocaleWeekInfo, this, "firstDay",
                       strlen("firstDay"), firstDay);

  zend_update_property(ecma_ce_IntlLocaleWeekInfo, this, "weekend",
                       strlen("weekend"), weekend);

  zend_update_property_long(ecma_ce_IntlLocaleWeekInfo, this, "minimalDays",
                            strlen("minimalDays"), minimalDays);
}
...
PHP_METHOD(Ecma_Intl_Locale_WeekInfo, jsonSerialize) {
  ...
  firstDay = *zend_read_property(ecma_ce_IntlLocaleWeekInfo, this, "firstDay",
                                 strlen("firstDay"), false, &rv);
  weekend = *zend_read_property(ecma_ce_IntlLocaleWeekInfo, this, "weekend",
                                strlen("weekend"), false, &rv);
  minimalDays =
      *zend_read_property(ecma_ce_IntlLocaleWeekInfo, this, "minimalDays",
                          strlen("minimalDays"), false, &rv);
