...
#define RETURN_PROPERTY(property)                                                                                      \
	do {                                                                                                               \
		...
		returnProperty(ecma_ce_IntlLocale, ECMA_LOCALE_P(getThis()), #property, return_value);                         \
	} while (0)

#define SET_PROPERTY_ARRAY(property, capacity)                                                                         \
	do {                                                                                                               \
		setPropertyArray(ecma_ce_IntlLocale, intlLocale, #property, capacity, ecma402_##property##OfLocale);           \
	} while (0)

#define SET_PROPERTY_STRING(property)                                                                                  \
	do {                                                                                                               \
		setPropertyString(ecma_ce_IntlLocale, intlLocale, #property, locale->property);                                \
	} while (0)
...
PHP_MINIT_FUNCTION(ecma_intl_locale) {
	...
	memcpy(&ecma_handlers_IntlLocale, zend_get_std_object_handlers(), sizeof(zend_object_handlers));
...
PHP_METHOD(Ecma_Intl_Locale, __construct) {
	...
	if (ecma402_hasError(locale->status)) {
		...
		SET_PROPERTY_ARRAY(numberingSystems, ECMA402_LOCALE_NUMBERING_SYSTEM_CAPACITY);
		zend_update_property_bool(ecma_ce_IntlLocale, object, "numeric", strlen("numeric"), locale->numeric);
...
static void maxOrMin(bool doMaximize, ecma_IntlLocale *locale, zval *dest) {
	...
	zend_call_method_with_1_params(Z_OBJ_P(dest), ecma_ce_IntlLocale, &ecma_ce_IntlLocale->constructor, "__construct",
	                               NULL, &arg1);
...
static void setTextInfo(zend_object *object, ecma402_locale *locale) {
	...
	directionObj = zend_enum_get_case_cstr(ecma_ce_IntlLocaleCharacterDirection, textDirection);
	...
	zend_update_property(ecma_ce_IntlLocaleTextInfo, textInfoObj, "direction", strlen("direction"), &direction);

	// Add the TextInfo object to this Locale instance.
	ZVAL_OBJ(&textInfo, textInfoObj);
	zend_update_property(ecma_ce_IntlLocale, object, "textInfo", strlen("textInfo"), &textInfo);
...
static void setWeekInfo(zend_object *object, ecma402_locale *locale) {
	...
	zend_enum_get_case_by_value(&firstDayObj, ecma_ce_IntlLocaleWeekDay, weekDayIcuToEcma(icuFirstDay), NULL, false);
	...
	for (int i = 0; i < weekendDaysCount; i++) {
		...
		zend_enum_get_case_by_value(&weekendDayObj, ecma_ce_IntlLocaleWeekDay, weekendDays[i], NULL, false);
	...
	zend_update_property(ecma_ce_IntlLocaleWeekInfo, weekInfoObj, "firstDay", strlen("firstDay"), &firstDay);
	zend_update_property(ecma_ce_IntlLocaleWeekInfo, weekInfoObj, "weekend", strlen("weekend"), &weekend);
	zend_update_property_long(ecma_ce_IntlLocaleWeekInfo, weekInfoObj, "minimalDays", strlen("minimalDays"),
	                          minimalDays);

	// Add the WeekInfo object to this Locale instance.
	ZVAL_OBJ(&weekInfo, weekInfoObj);
	zend_update_property(ecma_ce_IntlLocale, object, "weekInfo", strlen("weekInfo"), &weekInfo);
