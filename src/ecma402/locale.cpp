                                           ...
                                           const char *region, const char *script)
{
	...
	if (caseFirst != nullptr) {
		icuLocaleBuilder.setUnicodeLocaleKeyword(BCP47_KEYWORD_CASE_FIRST, caseFirst);
	}

	if (collation != nullptr) {
		icuLocaleBuilder.setUnicodeLocaleKeyword(BCP47_KEYWORD_COLLATION, collation);
	}
	...
	if (hourCycle != nullptr) {
		icuLocaleBuilder.setUnicodeLocaleKeyword(BCP47_KEYWORD_HOUR_CYCLE, hourCycle);
	}
	...
	if (numberingSystem != nullptr) {
		icuLocaleBuilder.setUnicodeLocaleKeyword(BCP47_KEYWORD_NUMBERING_SYSTEM, numberingSystem);
	}

	if (numeric == 1) {
		icuLocaleBuilder.setUnicodeLocaleKeyword(BCP47_KEYWORD_NUMERIC, BCP47_NUMERIC_TRUE);
	} else if (numeric == 0) {
		icuLocaleBuilder.setUnicodeLocaleKeyword(BCP47_KEYWORD_NUMERIC, BCP47_NUMERIC_FALSE);
	}
                                   ...
                                   ecma402_errorStatus *status)
{
	...
	for (i = 0; i < localesLength; i++) {
		...
		int const unicodeLocaleIdLength = ecma402_canonicalizeUnicodeLocaleId(locales[i], unicodeLocaleId, status);
...
int ecma402_canonicalizeUnicodeLocaleId(const char *localeId, char *canonicalized, ecma402_errorStatus *status)
{
	...
	if (!ecma402_isStructurallyValidLanguageTag(localeId)) {
		ecma402_ecmaError(status, STRUCTURALLY_INVALID_LOCALE_ID, "Invalid language tag \"%s\"", localeId);
		return -1;
	}
...
int ecma402_getBaseName(const char *localeId, char *baseName, ecma402_errorStatus *status, bool isCanonicalized)
{
	...
	icuBaseNameLength = getLocaleCode(localeId, icuBaseName, "base name", uloc_getBaseName, ULOC_FULLNAME_CAPACITY,
	                                  status, isCanonicalized);
...
int ecma402_getCurrency(const char *localeId, char *currency, ecma402_errorStatus *status, bool isCanonicalized)
{
	...
	if (cuPos != std::string::npos) {
		...
		size_t const cuLen = (endPos == std::string::npos) ? std::string::npos : endPos - startPos;
...
ecma402_locale *ecma402_initLocale(const char *localeId)
{
	...
	const int canonicalLength = ecma402_canonicalizeUnicodeLocaleId(localeId, canonical, locale->status);
	...
	INIT_PROPERTY(canonical, numberingSystem, ULOC_KEYWORDS_CAPACITY, getNumberingSystem);
...
bool ecma402_isNumeric(const char *localeId, ecma402_errorStatus *status, bool isCanonicalized)
{
	...
	length = getKeywordValue(ICU_KEYWORD_NUMERIC, localeId, numeric, status, isCanonicalized);
...
int ecma402_keywordsOfLocale(ecma402_locale *locale, const char *keyword, const char **values)
{
	...
	if (strcmp(keyword, ICU_KEYWORD_CALENDAR) != 0 && strcmp(keyword, ICU_KEYWORD_COLLATION) != 0 &&
	    strcmp(keyword, ICU_KEYWORD_CURRENCY) != 0 && strcmp(keyword, ICU_KEYWORD_HOUR_CYCLE) != 0 &&
	    strcmp(keyword, ICU_KEYWORD_NUMBERING_SYSTEM) != 0 && strcmp(keyword, ICU_KEYWORD_TIME_ZONE) != 0) {
		return 0;
	}
	...
	if (strcmp(keyword, ICU_KEYWORD_TIME_ZONE) == 0) {
		...
		if ((strcmp(keyword, ICU_KEYWORD_CURRENCY) == 0 && preferredLength == 3 && strcasecmp(preferred, "YES") != 0) ||
	...
	while ((name = uenum_next(items, nullptr, &icuStatus)) != nullptr) {
		...
		if (strcmp(keyword, ICU_KEYWORD_COLLATION) == 0 &&
		    (strcmp(name, ECMA402_COLLATION_SEARCH) == 0 || strcmp(name, ECMA402_COLLATION_STANDARD) == 0)) {
			continue;
		}
